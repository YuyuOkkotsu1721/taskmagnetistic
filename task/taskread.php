...
if (isset($_SESSION["userID"])) {
    ...
    if (isset($_GET['id'])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ?>
            <div id="tgrid" class="container mx-auto">
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <table class="min-w-full bg-white">
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-100 transform transition duration-300 ease-in-out hover:scale-105" <?php echo $bgColorStyle; ?>>
                                <td class="border px-4 py-2 w-1/6" <?php echo $textColorStyle; ?>>
                                    <input type="text" class="bg-transparent border-none outline-none w-full" id="TaskTitleInput"
                                        name="TaskTitleInput" value="<?php echo $row['TaskTitle']; ?>">
                                </td>
                                <td class="border px-4 py-2 w-1/6" <?php echo $textColorStyle; ?>>
                                    <textarea id="TaskDescriptionInput" name="TaskDescriptionInput" class="bg-transparent w-full" <?php echo $textColorStyle; ?>><?php echo $row['TaskDescription']; ?></textarea>
                                </td>
                                <td class="border px-4 py-2 w-1/12" <?php echo $textColorStyle; ?>>
                                    <?php echo date('m-d-Y h:i A', strtotime($row['TaskCreationDate'])); ?>
                                </td>
                                <td class="border px-4 py-2 w-1/12">
                                    <button class="w-full" style="color: <?php echo $row['TaskTextColor']; ?>;">
                                        <?php echo $row['CompletedSubtasks']; ?> / <?php echo $row['TotalSubtasks']; ?>
                                    </button>
                                </td>
                                ...
                                <td class="border px-4 py-2 text-center relative w-1/6">
                                    <!-- Added 'relative' to make the div a positioned container -->
                                    <div class="flex justify-center items-center space-x-2">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-6 w-6 text-white cursor-pointer absolute top-0 right-2"
   ...
   class="text-sm font-bold py-2 px-4 rounded"
   style="background-color: <?php echo htmlspecialchars($row['TaskTextColor']); ?>; color: <?php echo htmlspecialchars($row['TaskBackgroundColor']); ?>;"
   onclick="redirectToSubtask('<?php echo htmlspecialchars($row['TaskID']); ?>', '<?php echo htmlspecialchars($row['TaskTitle']); ?>')"
   oncontextmenu="setHrefForNewTab(this, '<?php echo htmlspecialchars($row['TaskID']); ?>', '<?php echo htmlspecialchars(addslashes($row['TaskTitle'])); ?>')">
   See Subtasks
</a>
                                    </div>
                                </td>
                        ...
                        <div class="absolute top-1/2 left-1/2 w-11/12 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 p-8 rounded-lg max-h-screen overflow-y-auto"
                            ...
                            <div class="mb-4" onclick="stopPropagation(event)">
                                ...
                                <textarea id="editTaskDescription"
                                    class="bg-gray-800 text-white px-4 py-2 w-full h-48 rounded focus:outline-none focus:ring-2 focus:ring-red-500">
                                                                                                        </textarea>
                            </div>
                            <div class="mb-4" onclick="stopPropagation(event)">
