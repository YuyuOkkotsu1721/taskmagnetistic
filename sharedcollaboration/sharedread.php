...
if (isset($_SESSION["userID"])) {
    ...
    // Assuming you have a way to get the current username, e.g., from the session
    $currentUsername = $_SESSION["username"]; // Adjust this based on how you store the username in the session

    // Prepare the SQL query
    $sql = "SELECT c.CollaboratorID, c.TaskID, c.CollaboratorMaker, c.SharedDateTime, 
    t.TaskTitle, t.TaskDescription, t.TaskDueDateTime, t.TaskPriority, 
    t.TaskCreationDate, t.TaskBackgroundColor, t.TaskTextColor,
    (SELECT COUNT(*) FROM subtasks WHERE TaskID = t.TaskID) AS TotalSubtasks,
    (SELECT COUNT(*) FROM subtasks WHERE TaskID = t.TaskID AND SubtaskStatus = 'Done') AS CompletedSubtasks
    FROM collaborators c 
    JOIN tasks t ON c.TaskID = t.TaskID 
    WHERE (c.CollaboratorMember = '$currentUsername' OR c.CollaboratorManager = '$currentUsername' )";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <div id="container_<?php echo $row['TaskID']; ?>"
                class="bg-blue-600 container mx-auto mt-8 p-6 md:p-12 rounded-xl relative" <?php echo $bgColorStyle; ?>>
                ...
                <div class="absolute top-0.5 left-1 mt-2 md:mt-4 ml-12 text-white font-medium text-sm" <?php echo $textColorStyle;
                ?>>
                    Shared by
                    <?php echo $row['CollaboratorMaker']; ?>
                </div>
                ...
                <div class="absolute top-0 right-0 mt-2 md:mt-4 mr-10 md:mr-16 text-white font-medium text-sm" <?php echo
                    $textColorStyle; ?>>
                    Shared on
                    <?php echo date('m-d-Y h:i A', strtotime($row['SharedDateTime'])); ?>
                </div>
                ...
                <div class="absolute top-0 right-0 mt-2 md:mt-4 mr-2 md:mr-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 md:h-8 w-6 md:w-8 text-white cursor-pointer"
                        ...
                        onclick="openEditModal('<?php echo $row['TaskID']; ?>', '<?php echo $row['TaskTitle']; ?>', 
            '<?php echo $row['TaskDescription']; ?>', '<?php echo $row['TaskDueDateTime']; ?>', 
            '<?php echo $row['TaskPriority']; ?>', '<?php echo $row['TaskBackgroundColor']; ?>', 
            '<?php echo $row['TaskTextColor']; ?>', '<?php echo $row['TotalSubtasks']; ?>', '<?php echo $currentUsername; ?>')">
                ...
                <!-- Additional Elements -->
                <div id="taskdetails"
                    class="absolute bottom-0 left-0 mb-2 md:mb-4 ml-2 md:ml-11 flex flex-wrap justify-center space-x-1">
                    <!-- Completion Button -->
                    <button class="bg-gray-700 font-medium text-white px-3 md:px-4 py-1 md:py-2 rounded-l-full"
                        style="background-color: <?php echo $row['TaskTextColor']; ?>; color: <?php echo $row['TaskBackgroundColor']; ?>;">
                        Completion:
                        <?php echo $row['CompletedSubtasks']; ?> /
                        <?php echo $row['TotalSubtasks']; ?>
                    </button>
                    <!-- Due Date Button -->
                    <button class="bg-gray-700 font-medium text-white px-3 md:px-4 py-1 md:py-2" id="dueDateButton"
                        style="background-color: <?php echo $row['TaskTextColor']; ?>; color: <?php echo $row['TaskBackgroundColor']; ?>;">Due
                        Date:
                        <?php echo date('m-d-Y h:i A', strtotime($row['TaskDueDateTime'])); ?>
                    </button>
                    <!-- Priority Button -->
                    <button class="bg-gray-700 font-medium text-white px-3 md:px-4 py-1 md:py-2 rounded-r-full" id="priorityButton"
                        style="background-color: <?php echo $row['TaskTextColor']; ?>; color: <?php echo $row['TaskBackgroundColor']; ?>;">
                        Priority:
                ...
                <button class="absolute bottom-0 right-0 mb-4 mr-4 font-bold py-2 px-4 rounded"
                    style="background-color: <?php echo $row['TaskTextColor']; ?>; color: <?php echo $row['TaskBackgroundColor']; ?>;"
                    onclick="redirectToSubtask('<?php echo $row['TaskID']; ?>', '<?php echo $row['TaskTitle']; ?>')">Next</button>
            </div>

            <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 hidden text-white" id="editmodal"
                onclick="stopPropagation(event)">
                <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 p-4 rounded-lg w-11/12 sm:w-4/5 md:w-3/5 lg:w-2/5 xl:w-1/3"
                    onclick="stopPropagation(event)">
                    <h2 class="text-2xl font-bold mb-4">Edit Task Settings</h2>
                    <p id="editcurrentUsername" class="text-white mb-4"></p>
                    ...
                    <label id="subtasktotals" class="block mb-2">Total Subtasks Remaining:</label>
                    ...
                    <input type="hidden" class="bg-purple-500 text-white" id="editTaskID" />
                    <div>
                        <button id="taskdeleteButton" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 hidden"
                            onclick="deleteTask()">Delete</button>
                        <div class="flex justify-end">


                            <button class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 mr-2 ml-auto"
        ...
        <script>
            ...
            function openEditModal(taskID, taskTitle, taskDescription, dueDate, priorityLevel, bgColor, textColor, totalSubtasks, currentUserUsername) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        // Check if currentUserUsername is present among the collaborators
                        var currentUserExists = false;
                        var collaborators = collaboratorList.getElementsByTagName('li');
                        for (var i = 0; i < collaborators.length; i++) {
                            if (collaborators[i].innerText.trim() === currentUserUsername) {
                                currentUserExists = true;
                                break;
                            }
                        }

                        // Only add delete buttons if currentUserUsername is not found among the collaborators
            ...
            function deleteCollaborator(taskID, collaboratorMember) {
                ...
                $.ajax({
                    ...
                    data: { collaboratorID: collaboratorID, collaboratorMember: collaboratorMember },
                    success: function (response) {
                        fetchCollaboratorsAndUpdateUI(taskID);
