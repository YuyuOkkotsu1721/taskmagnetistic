...
if (isset($_POST['subtaskID'])) {
    ...
    if ($status === "Done" || $status === "Done (For Review)") {
    ...
    if (in_array($status, ["Pending (Short-Term)", "Pending (Long-Term)", "Pending (To-Follow)", "Pending"])) {
        ...
        if (!empty($rowStartTimeCheck['SubtaskStartTime']) && $currentStatus === "Ongoing") {
            $sqlPausedDuration = "SELECT TIMEDIFF(NOW(), SUBTIME(SubtaskStartTime, '08:00:00')) AS PausedDuration FROM subtasks WHERE SubtaskID = '$subtaskID'";
            ...
            // Check if the paused duration is negative
            if (strpos($pausedDurationFormatted, '-') !== false) {
                // Remove the negative sign
                $pausedDurationFormatted = substr($pausedDurationFormatted, 1);
            }
    ...
    if (($currentStatusData['SubtaskStatus'] === "Done" || $currentStatusData['SubtaskStatus'] === "Done (For Review)") && in_array($status, ["Ongoing", "Pending (Short-Term)", "Pending (Long-Term)", "Pending (To-Follow)", "Pending"])) {
