    ...
    if (isset($_GET['taskid']) && !empty($_GET['taskid'])) {
        ...
        if ($clbstatus) {
            ...
            if (mysqli_num_rows($collaboratorSearchResultMaker) > 0) {
                ...
                $sql = "(SELECT s.SubtaskID, s.SubtaskTitle, s.SubtaskDescription, s.SubtaskDuration, s.SubtaskDifficulty, s.SubtaskCreationDate, s.SubtaskBackgroundColor, s.SubtaskTextColor, s.SubtaskStatus, s.SubtaskMarkNumber, s.SubtaskImage, s.SubtaskStartTime, s.SubtaskEndTime, s.SubtaskPausedDuration 
                        FROM subtasks AS s
                        INNER JOIN subtaskaccess AS sa ON s.SubtaskID = sa.SubtaskID AND s.TaskID = sa.TaskID
                        WHERE sa.AssignedTo = '{$_SESSION['username']}' AND s.TaskID = '$taskID')
                        UNION
                        (SELECT s.SubtaskID, s.SubtaskTitle, s.SubtaskDescription, s.SubtaskDuration, s.SubtaskDifficulty, s.SubtaskCreationDate, s.SubtaskBackgroundColor, s.SubtaskTextColor, s.SubtaskStatus, s.SubtaskMarkNumber, s.SubtaskImage, s.SubtaskStartTime, s.SubtaskEndTime, s.SubtaskPausedDuration 
                        FROM subtasks AS s
                        WHERE s.TaskID = '$taskID'
                        AND CONCAT(s.SubtaskID, 'ed0') IN (SELECT EditLogID FROM editlogs WHERE UserID = '" . $_SESSION["userID"] . "'))";
            } else {
        ...
        if (mysqli_num_rows($result) > 0) {
                        ...
                        <div id="subtaskcontent"
                            ...
                            <?php if (!empty($assignedTo)) { ?>
                                <p style="color: <?php echo $row['SubtaskTextColor']; ?>;" id="assignedName"
                                    class="text-xs font-bold absolute bottom-7 right-6 ">
                                    <?php echo $assignedTo; ?>
                                </p>
                            <?php } ?>
                            <?php } ?>
            ...
            <div id="myalertModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
                <div class="bg-white rounded shadow-md w-1/2">
                    ...
                    <p class="ml-3 text-purple-700" id="alertText" class="p-3"></p>
                ...
                <div class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 p-4 rounded-lg w-11/12 "
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="rounded-lg py-2 cursor-pointer hover:bg-gray-500 border border-transparent ">
                            <h2 id="editsubtasklabel" class="text-white text-2xl font-bold">Edit Subtask Settings</h2>
                        </div>
            ...
            <div id="chatModal"
                ...
                <div class="bg-white text-black rounded-lg shadow-lg w-full">
                    ...
                    <span id="chatheaderlabel" class="absolute top-0 left-2 text-3xl font-bold text-white text-left">Chat Up</span>
            ...
            <div id="visualModal" class="modal fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center">
                <div id="visualModalcontent" class="bg-gray-800  mx-auto overflow-hidden relative rounded-2xl"
                    ...
                    <?php
                    $hideRingCircle = ($clbstatus) ? '' : 'hidden'; // Determine if the ringcircle should be hidden
                    ?>

                    <div id="ringcircle"
                        class="absolute bottom-16 right-6 p-2 font-medium rounded-full bg-yellow-500 cursor-pointer <?php echo $hideRingCircle; ?>"
...
setInterval(fetchBuzzChatsread, 1000);
                ...
                function accessadd() {
                    ...
                    if (!collaborator) {
                        openAlertModal("Please select a collaborator.");
                ...
                function openEditModal(subtaskID, subtaskTitle, subtaskDescription, subtaskDuration, subtaskDifficulty, bgColor, textColor, status, markNumber) {
                    ...
                    if (status === 'Ongoing' || status === 'Done' || status === 'Done (For Review)') {
                        document.getElementById('editStatus').querySelector("option[value='Done']").style.display = 'block';
                        document.getElementById('editStatus').querySelector("option[value='Done (For Review)']").style.display = 'none';
                ...
                function logUpdatedInputs() {
                    ...
                    // Here we can trigger an event or directly embed JavaScript code to execute
                    document.dispatchEvent(new CustomEvent('logUpdatedInputsTriggered', {
                        detail: {
                            formData: formData,
                            message: 'logUpdatedInputs was triggered and processed'
                        }
                    }));
                }
